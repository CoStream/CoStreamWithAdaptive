...
public class SimpleJoinerWithSubWindow<F, S> extends CommonJoiner<F, S> {

    //采用子索引的方式存储元组，格式为 最大时间戳，最小时间戳，索引
    List<Tuple3<Long, Long, TreeMap<Double, List<CommonJoinUnionType<F, S>>>>> R_Tree_set;
    List<Tuple3<Long, Long, TreeMap<Double, List<CommonJoinUnionType<F, S>>>>> S_Tree_set;

    //每个子树的时间范围（此处与窗口大小相同）
    private long subTreeDuration = R_TimeWindows.toMilliseconds() / 2;
    ...
    //获取logger对象用于日志记录
    private static final Logger logger = Logger.getLogger(SimpleJoinerWithSubWindow.class.getName());

    /**
     * 有参构造器，进行范围连接的相关初始化参数设置
     */
    public SimpleJoinerWithSubWindow(Time r_TimeWindows, Time s_TimeWindows, KeySelector<F, Double> keySelector_R, KeySelector<S, Double> keySelector_S, double r_surpass_S, double r_behind_S) {
    ...
    public void open(Configuration parameters) throws Exception {
        ...
        R_Tree_set.add(new Tuple3<>(Long.MIN_VALUE, Long.MAX_VALUE, new TreeMap<>()));
        S_Tree_set = new LinkedList<>();
        S_Tree_set.add(new Tuple3<>(Long.MIN_VALUE, Long.MAX_VALUE, new TreeMap<>()));
        //初始化上一次过期的时间
        lastExpireTime = 0L;

        logger.info(this.getClass().getName() + "-SimpleJoinerWithSubWindow-" + subTaskIdx + ".启动成功！");
    }
    ...
    private void removeExpireTuplesOfTree(
            List<Tuple3<Long, Long, TreeMap<Double, List<CommonJoinUnionType<F, S>>>>> storeTreeSet,
            long minTimestamp) {
        //使用迭代器删除，防止出现并发修改错误
        Iterator<Tuple3<Long, Long, TreeMap<Double, List<CommonJoinUnionType<F, S>>>>> iterator = storeTreeSet.iterator();
        while (iterator.hasNext()) {
            Tuple3<Long, Long, TreeMap<Double, List<CommonJoinUnionType<F, S>>>> next = iterator.next();
    ...
    private void storeTuple(CommonJoinUnionType<F, S> value) throws Exception {
        if (value.isOne()) {
            Tuple3<Long, Long, TreeMap<Double, List<CommonJoinUnionType<F, S>>>> storeRTree = R_Tree_set.get(0);
            ...
            if (storeRTree.f0 - storeRTree.f1 >= subTreeDuration) {
                R_Tree_set.add(0, new Tuple3<>(Long.MIN_VALUE, Long.MAX_VALUE, new TreeMap<>()));
                logger.info("R树新建子树，触发新建子树的元组的时间戳为：" + value.getSelfTimestamp());
            }

        } else {
            Tuple3<Long, Long, TreeMap<Double, List<CommonJoinUnionType<F, S>>>> storeSTree = S_Tree_set.get(0);
            ...
            if (storeSTree.f0 - storeSTree.f1 >= subTreeDuration) {
                S_Tree_set.add(0, new Tuple3<>(Long.MIN_VALUE, Long.MAX_VALUE, new TreeMap<>()));
                logger.info("S树新建子树，触发新建子树的元组的时间戳为：" + value.getSelfTimestamp());
            }
        }
    }
    ...
    private void joinTuples(CommonJoinUnionType<F, S> value, Collector<CommonJoinUnionType<F, S>> out) throws Exception {
        if (value.isOne()) {  //R流元组
            ...
            for (Tuple3<Long, Long, TreeMap<Double, List<CommonJoinUnionType<F, S>>>> joinTree : S_Tree_set) {
            ...
            for (Tuple3<Long, Long, TreeMap<Double, List<CommonJoinUnionType<F, S>>>> joinTree : R_Tree_set) {
