...
/**
 * 采用链式子窗口索引的Joiner，其中每个子窗口中元组的数量是固定的，主要用于比较BiModel，每个子索引中均是B+树类型
 * 只能处理等值连接
 */
public class JoinerWithSubBPlusTree<F, S> extends CommonJoiner<F, S> {

    //采用子索引的方式存储元组，格式为-- 最大时间戳，最小时间戳，索引, 子索引元组数量
    List<Tuple4<Long, Long, SimpleSubBPlusTree<CommonJoinUnionType<F, S>, Double>, Long>> R_Tree_set;
    List<Tuple4<Long, Long, SimpleSubBPlusTree<CommonJoinUnionType<F, S>, Double>, Long>> S_Tree_set;
    ...
    //获取logger对象用于日志记录
    private static final Logger logger = Logger.getLogger(JoinerWithSubBPlusTree.class.getName());

    /**
     * 有参构造器，进行范围连接的相关初始化参数设置
     */
    public JoinerWithSubBPlusTree(Time r_TimeWindows, Time s_TimeWindows, KeySelector<F, Double> keySelector_R, KeySelector<S, Double> keySelector_S, double r_surpass_S, double r_behind_S) {
    ...
    public void open(Configuration parameters) throws Exception {
        ...
        R_Tree_set.add(new Tuple4<>(Long.MIN_VALUE, Long.MAX_VALUE, new SimpleSubBPlusTree<>(), 0L));
        S_Tree_set = new LinkedList<>();
        S_Tree_set.add(new Tuple4<>(Long.MIN_VALUE, Long.MAX_VALUE, new SimpleSubBPlusTree<>(), 0L));
        //初始化上一次过期的时间
        lastExpireTime = 0L;
    ...
    private void expire(long currentWatermark) {
        ...
        //如果对应的R树不为空才开始执行过期 TODO 打印位置0数量是为了统计元组实时数量变化
        logger.info(">>>无论此Joiner是否存储R元组，Joiner-" + subTaskIdx + ":R树过期前size为-" + R_Tree_set.size() + ";位置0的子索引的总元组数量为：" + (R_Tree_set.get(0).f3));
        ...
        //如果对应的S树不为空才开始执行过期
        logger.info(">>>无论此Joiner是否存储S元组，Joiner-" + subTaskIdx + ":S树过期前size为-" + S_Tree_set.size() + ";位置0的子索引的总元组数量为：" + (S_Tree_set.get(0).f3));
        if (S_Tree_set.size() > 1) {
            logger.info(">>>Joiner-" + subTaskIdx + ":S树过期前size为-" + S_Tree_set.size() + ";位置0的子索引的总元组数量为：" + (S_Tree_set.get(0).f3));
            //计算要保留的最小时间戳
            long minS_Timestamp = currentWatermark - S_TimeWindows.toMilliseconds();
            //遍历，删除小于最小时间戳的项
            removeExpireTuplesOfTree(S_Tree_set, minS_Timestamp);

            logger.info(">>>Joiner-" + subTaskIdx + ":S树过期后size为-" + S_Tree_set.size() + ";位置0的子索引的总元组数量为：" + (S_Tree_set.get(0).f3));
        }
    ...
    private void removeExpireTuplesOfTree(
            List<Tuple4<Long, Long, SimpleSubBPlusTree<CommonJoinUnionType<F, S>, Double>, Long>> storeTreeSet,
            long minTimestamp) {
        //使用迭代器删除，防止出现并发修改错误
        Iterator<Tuple4<Long, Long, SimpleSubBPlusTree<CommonJoinUnionType<F, S>, Double>, Long>> iterator = storeTreeSet.iterator();
        while (iterator.hasNext()) {
            Tuple4<Long, Long, SimpleSubBPlusTree<CommonJoinUnionType<F, S>, Double>, Long> next = iterator.next();
    ...
    private void storeTuple(CommonJoinUnionType<F, S> value) throws Exception {
        ...
        //防止插入之前两棵树因为过期而为空
        if (R_Tree_set.isEmpty()) {
            R_Tree_set.add(new Tuple4<>(Long.MIN_VALUE, Long.MAX_VALUE, new SimpleSubBPlusTree<>(), 0L));
        }
        if (S_Tree_set.isEmpty()) {
            S_Tree_set.add(new Tuple4<>(Long.MIN_VALUE, Long.MAX_VALUE, new SimpleSubBPlusTree<>(), 0L));
        }

        if (value.isOne()) {
            Tuple4<Long, Long, SimpleSubBPlusTree<CommonJoinUnionType<F, S>, Double>, Long> storeRTree = R_Tree_set.get(0);
            ...
            if (storeRTree.f3 >= subTreeNum) {
                R_Tree_set.add(0, new Tuple4<>(Long.MIN_VALUE, Long.MAX_VALUE, new SimpleSubBPlusTree<>(), 0L));
                logger.info("R树新建子树，触发新建子树的元组的时间戳为：" + value.getSelfTimestamp() + ";上一个活动子树元组数量为：" + storeRTree.f3);
            }

        } else {
            Tuple4<Long, Long, SimpleSubBPlusTree<CommonJoinUnionType<F, S>, Double>, Long> storeSTree = S_Tree_set.get(0);
            ...
            if (storeSTree.f3 >= subTreeNum) {
                S_Tree_set.add(0, new Tuple4<>(Long.MIN_VALUE, Long.MAX_VALUE, new SimpleSubBPlusTree<>(), 0L));
     ...
     * @param value 要存储的元组
     */
    private void storeTupleToSubTree(CommonJoinUnionType<F, S> value, SimpleSubBPlusTree<CommonJoinUnionType<F, S>, Double> storeTree) throws Exception {
        //根据元组所属流的不同存储于不同的树
        Double key = 0.0;
        if (value.isOne()) { //R流
        ...
        storeTree.insert(value, key, value.getSelfTimestamp());
    ...
    private void joinTuples(CommonJoinUnionType<F, S> value, Collector<CommonJoinUnionType<F, S>> out) throws Exception {
        if (value.isOne()) {  //R流元组
            ...
            for (Tuple4<Long, Long, SimpleSubBPlusTree<CommonJoinUnionType<F, S>, Double>, Long> joinTree : S_Tree_set) {
            ...
            for (Tuple4<Long, Long, SimpleSubBPlusTree<CommonJoinUnionType<F, S>, Double>, Long> joinTree : R_Tree_set) {
    ...
    private List<CommonJoinUnionType<F, S>> joinTuplesWithSubTree(
            ...
            SimpleSubBPlusTree<CommonJoinUnionType<F, S>, Double> joinTree
    ) throws Exception {
        ...
        if (value.isOne()) {  // 如果为R流元组，则需要与S的树进行连接
            ...
            long min_S_Timestamp = value.getSelfTimestamp() - S_TimeWindows.toMilliseconds();
            List<CommonJoinUnionType<F, S>> S_MatchedList = joinTree.findRange(key, key, min_S_Timestamp);
            if (S_MatchedList == null) {
                return null;
            }
            resultList.addAll(S_MatchedList);

        } else {  // 如果为S流元组,需要与R的元组进行连接
            Double S_Key = keySelector_S.getKey(value.getSecondType());
            long min_R_Timestamp = value.getSelfTimestamp() - R_TimeWindows.toMilliseconds();
            List<CommonJoinUnionType<F, S>> R_MatchedList = joinTree.findRange(S_Key, S_Key, min_R_Timestamp);
            if (R_MatchedList == null) {
                return null;
            }
            resultList.addAll(R_MatchedList);
