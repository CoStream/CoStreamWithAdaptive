...
import coModel.adaptive.CoModelUnionStreamWithAdaptive;
import coModel.adaptive.index.CommonJoinUnionTypeSerializerForTuple4ForAdaptive;
import coModel.tools.sync.ZookeeperBasedSynchronizer;
import common.GeneralParameters;
import experiment.component.ComputeResultTupleLatencyMapFunctionPrecise;
import experiment.component.MyCommonTestWatermarkPeriodicAssigner;
import experiment.component.ParallelAccumulateResultLatencyMapFunctionPrecise;
 ...
 * 此实验为窗口为10分钟的等值连接
 * 此实验中采用的统计计算结果延迟的方式是直接获取系统时间，因此计算的延迟是精确的
 */
public class ExperimentWithAdaptiveExecutorForNEXMark implements Serializable {
    //用于标识是在进行哪个实验
    ...
    //获取logger对象用于日志记录
    private static final Logger logger = Logger.getLogger(ExperimentWithAdaptiveExecutorForNEXMark.class.getName());
     ...
     * @param args 程序的输入参数
     */
    public ExperimentWithAdaptiveExecutorForNEXMark(String experimentModel, String[] args) {
    ...
    public void startExperiment() throws Exception {
        ...
        logger.info(">>>开始执行ExperimentExecutorForNEXMark实验");
        System.out.println("在该实验中，采用的是CoModel模型，且在CoModel中基于非迁移方式实现复制数调整");
        System.out.println("当前程序接收的参数种类：--rCopyNum --sCopyNum --R_surpass_S --R_behind_S");
        ...
        //构建合并的数据流
        CommonJoinUnionStream<Tuple4<String, String, String, String>, Tuple4<String, String, String, String>> myUnionStream =
                new CoModelUnionStreamWithAdaptive<>(firstStream, secondStream, keySelector, keySelector, new CommonJoinUnionTypeSerializerForTuple4ForAdaptive());
        logger.info(">>>Experiment当前为CoModel,当前程序使用堆外内存减少垃圾回收开销，且CoModel通过非迁移方式实现复制数调整");


        //进行连接，产生结果流
        DataStream<CommonJoinUnionType<Tuple4<String, String, String, String>, Tuple4<String, String, String, String>>> resultStream =
                myUnionStream.bandNMJoinWithTimeWindow(
                        Time.seconds(GeneralParameters.WINDOW_SIZE),
                        Time.seconds(GeneralParameters.WINDOW_SIZE),
                        r_surpass_s,
                        r_behind_s);
        ...
        //结果处理
        resultStream
                .map(new ComputeResultTupleLatencyMapFunctionPrecise())         //计算每个结果元组的延迟(精确)
                .setParallelism(GeneralParameters.TOTAL_NUM_OF_TASK)    //需要设置并行度与Joiner数量一致
                .flatMap(new ParallelAccumulateResultLatencyMapFunctionPrecise())    //在每个并行实例上分别累计每一秒的延迟（精确）
                .setParallelism(GeneralParameters.TOTAL_NUM_OF_TASK)    //需要设置并行度与Joiner数量一致
