...
/**
 * 采用堆外内存作为内存池的子索引结构,包含自适应功能
 * @param <K> : 存储的键值类型
 */
public class JoinerSubIndexWithOffHeapPoolForAdaptive<F, S, K extends Comparable<K>> implements Serializable {
    ...
    /**
     * 初始化,不指定当前的复制数和行号，此时与所有的连接元组进行连接
     * @param maxSize 该子索引存储元组数量的上限（单位：字节数）
     * @param serializer 元组堆外内存序列化器
     */
    public JoinerSubIndexWithOffHeapPoolForAdaptive(long maxSize, CommonJoinUnionTypeSerializerForAdaptive<F, S> serializer) {
        this.maxSize = maxSize;
        this.serializer = serializer;
        initAddress = unsafe.allocateMemory(maxSize);
        reset();
        //此时所有到达的连接元组均需要与本地进行连接
        this.copyNum = 1;
        this.rowNum = 0;
    }

    /**
     * 初始化，指定复制数和行号的版本，此时只与指定范围内的连接元组进行连接
     * @param maxSize 该子索引存储元组数量的上限（单位：字节数）
     * @param serializer 元组堆外内存序列化器
     * @param copyNum 当前子索引对应的系统的复制数（用于判断哪些连接元组需要与本子索引进行连接）
     * @param rowNum 当前子索引对应的节点在当前复制数下的行号（用于判断哪些连接元组需要与本子索引进行连接）
     */
    public JoinerSubIndexWithOffHeapPoolForAdaptive(long maxSize, CommonJoinUnionTypeSerializerForAdaptive<F, S> serializer, int copyNum, int rowNum) {
        this.maxSize = maxSize;
        this.serializer = serializer;
        initAddress = unsafe.allocateMemory(maxSize);
        reset();
    ...
    /**
     * 根据到达元组的序列号判断该元组是否应该和该子索引进行连接
     * @param serialNum 新到达连接元组的序列号
     * @return 如果需要与本地进行连接，则返回：True；
     */
    public boolean isCurrentSerialNumMatchLocal(long serialNum) {
        return serialNum % copyNum == rowNum;
    }
    ...
    /**
     * 获取该子索引存储的所有元组的时间戳的范围（即最大时间戳和最小时间戳的差）(ms)
     */
    ...
    /**
     * 重置当前子树（之后可以向该子树中重新插入元组，相当于删除当前子树，但实际本地的存储结构未做任何改变）
     * 同时为重置后的子树设置序列号范围，只有满足范围要求的连接元组会与本地进行连接
     * @param copyNum 当前系统对应流的复制数
     * @param rowNum 当前Joiner对应的行号
     */
    public void resetWithSpecialSerialRange(int copyNum, int rowNum) {
        reset();
        this.copyNum = copyNum;
        this.rowNum = rowNum;
    }
