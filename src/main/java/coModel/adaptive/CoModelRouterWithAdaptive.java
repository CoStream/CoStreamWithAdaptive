...
public class CoModelRouterWithAdaptive<F, S> extends CommonRouter<F, S> {

    //系统中总的Router数量与Joiner数量
    private int totalRouterNum;
    private int totalJoinerNum;

    //系统中用于存储R与S的Joiner数量
    private int R_totalStoreNum;
    private int S_totalStoreNum;
    //系统中在R（或S）的所有存储节点中，每个R（或S）元组存储的副本数量
    //这两个值用于控制存储元组的转发，而不用于控制连接元组的转发，二者在自适应改变复制数时有区别
    private int R_copiesNumForStore;
    private int S_copiesNumForStore;
    //这两个值用于控制连接元组的转发，而不用于控制存储元组的转发，二者在自适应改变复制数时有区别
    private int R_copiesNumForJoin;
    private int S_copiesNumForJoin;

    //R中元组与S中存储元组的序列号（用于指导存储流分区，序列号最大数量为-对应-流的存储节点数量，并且每个元组到达时序列号依次递增）
    private int currentSerialNumOf_Store_R = 0;
    private int currentSerialNumOf_Store_S = 0;
    //R中元组与S中连接元组的序列号（用于指导连接流分区，序列号最大数量为-相对-流的存储节点数量，并且每个元组到达时序列号依次递增）
    private int currentSerialNumOf_Join_R = 0;
    private int currentSerialNumOf_Join_S = 0;
    ...
    //用于唤醒Router的条件
    private Condition wakeUpRouterCondition = lock.newCondition();
    ...
    //===================================================输入速率监控组件===============================================
    //Router中要用到的输入速率信息上传定时器
    private final MyTimer informationUploadTimer = new MyTimer();
    //Router的定时器中定时执行的信息上传的定时任务
    private MyTimerTask informationUploadTimerTask = null;
    // 输入速率的监控与上传周期(单位：ms)
    private long inputRateMonitorPeriod = CoModelParameterWithAdaptive.ROUTER_INPUT_RATE_MONITOR_AND_UPLOAD_PERIOD;
    // R与S的输入速率（严格来说该值不直接代表速度，而是代表每个监控周期内的累计数，需要除以监控周期）
    private long inputRateOf_R = 0;
    private long inputRateOf_S = 0;
    // 输入速率是否重置的表示。True：应当重置输入速率
    private boolean isInputRateShouldReset = false;

    //=============================================本地路由方案延迟变更组件===============================================
    //Router中路由方案延迟变更的定时器
    private final MyTimer routeChangeTimer = new MyTimer();
    //Router的定时器中定时执行的路由方案变更定时任务
    private MyTimerTask routeSchemaChangeTimerTask = null;


    //当前Router的状态
    private final RouterStatus routerState = new RouterStatus();

    //用于处理信号消息的对象
    private final SignalMessageFactoryForCoModel<F, S> signalFactory = new SignalMessageFactoryForCoModel<>();

    //获取logger对象用于日志记录
    private static final Logger logger = Logger.getLogger(CoModelRouterWithAdaptive.class.getName());


    /**
     * 通用的有参构造
     */
    public CoModelRouterWithAdaptive(Time r_TimeWindows, Time s_TimeWindows, double r_surpass_S, double r_behind_S, KeySelector<F, Double> keySelector_R, KeySelector<S, Double> keySelector_S) {
    ...
    public void open(Configuration parameters) throws Exception {
        ...
        //初始化周期性输入速率上传监视器
        startOrResetInformationTimer(5000, inputRateMonitorPeriod);

        //打印当前系统状态
        logCoModelCurrentStatus(R_totalStoreNum, S_totalStoreNum, R_copiesNumForStore, S_copiesNumForStore, R_copiesNumForJoin, S_copiesNumForJoin);

        logger.info("CoModel-Router-" + subTaskNum + ":系统初始化完成！当前系统能够进行自适应！");
    ...
    private void initParameters() {
        ...
        // 复制数采用Zookeeper中的配置
        R_copiesNumForStore = Integer.parseInt(
        ...
        S_copiesNumForStore = Integer.parseInt(
                ParseExperimentParametersTool
                        .parseExpParametersForNameWithinByteArray(argsBytes, ParseExperimentParametersTool.PARAM_NAME_OF_S_COPY_NUM));
        //初始连接元组的控制方式与存储元组相同
        R_copiesNumForJoin = R_copiesNumForStore;
        S_copiesNumForJoin = S_copiesNumForStore;
    ...
    private void initCoordinatorInSpecialRouter(int routerNum) {
        if (subTaskNum == routerNum) {
            ...
            new CoModelCoordinatorWithAdaptive().start();
            logger.info("CoModel-Router-" + subTaskNum + ":启动中央协调器(自适应)成功！");
        }
    }
    ...
    /**
     * 初始化一个信息上传定时器，或者重置现有信息上传定时器（主要上传当前的速率信息）
     * @param initDelay 内存监视器第一次的执行时间(单位：ms)
     * @param period 内存监视器以后的周期性执行时间(单位：ms)
     */
    private void startOrResetInformationTimer(long initDelay, long period) {

        //上传速率(该句也用于初始化创建节点)
        synchronizer.setZookeeperNodeContent(ZookeeperNodePathSetForCoModel.ROUTER_MONITOR_TWO_STREAM_INPUT_RATE_PREFIX + subTaskNum,
                ("" + inputRateOf_R + "," + inputRateOf_S).getBytes());

        //关闭上一个定时任务
        if (informationUploadTimerTask != null) {
            informationUploadTimerTask.cancel();
        }

        //新建定时任务
        informationUploadTimerTask = new MyTimerTask() {
            @Override
            public void run() {
                //周期性的上传监控到的两个流的输入速率
                periodInputRateMonitorAndUpload();
            }
        };

        //调度新的定时任务
        informationUploadTimer.schedule(informationUploadTimerTask, initDelay, period);
        logger.info("CoModel-Router-" + subTaskNum
                + ":初始化/重置了一个输入速率监控线程，线程将在：" + initDelay
     ...
     * 周期性的上传监控到的两个流的输入速率
     */
    private void periodInputRateMonitorAndUpload() {
        //上传速率
        synchronizer.setZookeeperNodeContent(ZookeeperNodePathSetForCoModel.ROUTER_MONITOR_TWO_STREAM_INPUT_RATE_PREFIX + subTaskNum,
                ("" + inputRateOf_R + "," + inputRateOf_S).getBytes());
        //重置速率
        isInputRateShouldReset = true;
    }
     ...
     * @param currentCopiesNumOf_R_Join 控制连接元组的转发数
     * @param currentCopiesNumOf_S_Join 控制连接元组的转发数
     */
    private void logCoModelCurrentStatus(int totalStoreNumOf_R, int totalStoreNumOf_S,
                                         int currentCopiesNumOf_R, int currentCopiesNumOf_S,
                                         int currentCopiesNumOf_R_Join, int currentCopiesNumOf_S_Join) {
        logger.info("CoModel-Router-" + subTaskNum
                ...
                + ";用于控制连接元组转发的R的副本数为-" + currentCopiesNumOf_R_Join
                + ";用于控制连接元组转发的S的副本数为-" + currentCopiesNumOf_S_Join
                + "。");
    }
    ...
    private void notifyJoinerSyncCompleteMessHandler(TransferProtocolBasedZookeeper receiveMess) {
        ...
        try {
            ...
            //尝试变更连接分区(R流)
            if (R_copiesNumForStore < newPartitionScheme.f1) {  // 如果新方案是存储数增加的方案，则连接分区延迟变化
                logger.info("CoModel-Router-" + subTaskNum + ":R的存储分区将要增加，因此延迟更新R的连接分区");
                // 延迟的时间为一整个事件窗口的事件，在额外加上1个子窗口时间的冗余
                long lateInterval = R_TimeWindows.toMilliseconds() + (R_TimeWindows.toMilliseconds() / CoModelParameterWithAdaptive.NUM_OF_SUB_INDEX);
                startOrResetRouteSchemaChangeTimer(lateInterval);
            } else {  // 如果新方案的存储数减小或不变，则直接更新连接分区方案
                R_copiesNumForJoin = newPartitionScheme.f1;
            }
            //尝试变更连接分区(S流)
            if (S_copiesNumForStore < newPartitionScheme.f3) {  // 如果新方案是存储数增加的方案，则连接分区延迟变化
                logger.info("CoModel-Router-" + subTaskNum + ":S的存储分区将要增加，因此延迟更新S的连接分区");
                // 延迟的时间为一整个事件窗口的事件，在额外加上两个子窗口时间的冗余
                long lateInterval = S_TimeWindows.toMilliseconds() + (S_TimeWindows.toMilliseconds() / CoModelParameterWithAdaptive.NUM_OF_SUB_INDEX);
                startOrResetRouteSchemaChangeTimer(lateInterval);
            } else {  // 如果新方案的存储数减小或不变，则直接更新连接分区方案
                S_copiesNumForJoin = newPartitionScheme.f3;
            }

            //用新的分区方案代替本地的(存储)分区方案
            //注意：无论是什么情况，存储分区均是被直接更改的
            R_totalStoreNum = newPartitionScheme.f0;
            R_copiesNumForStore = newPartitionScheme.f1;
            S_totalStoreNum = newPartitionScheme.f2;
            S_copiesNumForStore = newPartitionScheme.f3;
    ...
    /**
     * 初始化一个路由方案变更定时器，或者重置现有路由方案变更定时器
     * 采用事件时间会更具有一般性，但对于我的实验来说，二者近似
     * 该方法的作用是当复制数增加时，为了保证连接结果的正确性，在一个窗口多一些的时间内，连接元组的转发方式还是按照原来的方案
     * @param initDelay 第一次的执行时间(单位：ms)
     */
    private void startOrResetRouteSchemaChangeTimer(long initDelay) {

        //关闭上一个定时任务
        if (routeSchemaChangeTimerTask != null) {
            routeSchemaChangeTimerTask.cancel();
        }

        //新建定时任务
        routeSchemaChangeTimerTask = new MyTimerTask() {
            @Override
            public void run() {
                //延迟路由变更
                lateRouteSchemaChange();
            }
        };

        //调度新的定时任务
        routeChangeTimer.schedule(routeSchemaChangeTimerTask, initDelay);
        logger.info("CoModel-Router-" + subTaskNum
                + ":初始化/重置了一个(连接)路由方案变更定时器，线程将在：" + initDelay + "ms 时间后被执行");
    }

    /**
     * 在Router接收到路由变更消息且消息表示要增大复制数时，该方法在一段时间后被调用，以变更连接元组的转发方案，保证结果正确性
     */
    private void lateRouteSchemaChange() {
        R_copiesNumForJoin = newPartitionScheme.f1;
        S_copiesNumForJoin = newPartitionScheme.f3;
        logger.info("CoModel-Router-" + subTaskNum + ":延迟连接分区变更成功！");
        //打印当前系统状态
        logCoModelCurrentStatus(R_totalStoreNum, S_totalStoreNum, R_copiesNumForStore, S_copiesNumForStore, R_copiesNumForJoin, S_copiesNumForJoin);
    }
    ...
    protected List<Integer> getStorePartitionNumList(int totalStoreJoinerNum, int currentCopiesNum, int tupleSerialNum){
        ...
        //当前流被分了多少组
        int totalGroupNum = totalStoreJoinerNum / currentCopiesNum;

        // 当前序列号属于第几组
        int groupNum = tupleSerialNum % totalGroupNum;
    ...
    /**
     * 在处理每个输入元组时，计算该元组对应流的输入速率
     * @param value
     */
    private void monitorInputRate(CommonJoinUnionType<F, S> value) {
        //重置上一监控周期的输入速率
        if (isInputRateShouldReset) {
            inputRateOf_R = 0;
            inputRateOf_S = 0;
            isInputRateShouldReset = false;
        }
        //输入速率加一
        if (value.isOne()) {
            inputRateOf_R++;
     ...
     * 处理R流当中的元组
     */
    private void processElementOf_R(CommonJoinUnionType<F, S> value, Context ctx, Collector<CommonJoinUnionType<F, S>> out) {
        //为当前元组设置序列号，并使序列号递增
        //--取余操作最好在前面，防止越界
        ...
        //计算存储分区列表
        List<Integer> storePartitionNumList = getStorePartitionNumList(R_totalStoreNum, R_copiesNumForStore, value.getSerialNumOfCoModel());
        ...
        List<Integer> joinPartitionNumList = getJoinPartitionNumList(S_totalStoreNum, S_copiesNumForJoin, currentSerialNumOf_Join_R);
     ...
     * 处理S流当中的元组
     */
    private void processElementOf_S(CommonJoinUnionType<F, S> value, Context ctx, Collector<CommonJoinUnionType<F, S>> out) {
        //为当前元组设置序列号，并使序列号递增
        ...
        //计算存储分区列表
        List<Integer> storePartitionNumList = getStorePartitionNumList(S_totalStoreNum, S_copiesNumForStore, value.getSerialNumOfCoModel());
        ...
        List<Integer> joinPartitionNumList = getJoinPartitionNumList(R_totalStoreNum, R_copiesNumForJoin, currentSerialNumOf_Join_S);
