...
/**
 * 基础的Joiner类，其中未对具体的存储结构做出任何定义，可以自行实现任意的存储结构
 ...
 * @param <S> 第二个流的类型
 */
public abstract class AbstractCoModelCommonJoiner<F, S> extends CommonJoiner<F, S> {
    ...
    //获取logger对象用于日志记录
    protected static final Logger logger = Logger.getLogger(AbstractCoModelCommonJoiner.class.getName());


    /**
     * 有参构造器，进行范围连接的相关初始化参数设置
     */
    public AbstractCoModelCommonJoiner(Time r_TimeWindows, Time s_TimeWindows, KeySelector<F, Double> keySelector_R, KeySelector<S, Double> keySelector_S, double r_surpass_S, double r_behind_S) {
    ...
    /**
     * 判断当前Joiner是否是R流中元组的存储节点
     * @param RStoreNodeNum R的存储节点数量
     * @return 如果本地Joiner是用于存储R流中的元组的，返回true
     */
    protected boolean isLocalJoinerTheRStoreNode(int RStoreNodeNum){
        return subTaskIdx < RStoreNodeNum;
    }
    ...
    protected void probeTupleAndOutput(CommonJoinUnionType<F, S> value, Context ctx, Collector<CommonJoinUnionType<F, S>> out) throws Exception {
        if (value.isOne()) {  // R中的元组，要与S的存储结构连接
            ...
            List<CommonJoinUnionType<F, S>> resultList = getAllMatchedTuplesForEachStream(minTimestamp, minKey, maxKey, false);
            ...
            List<CommonJoinUnionType<F, S>> resultList = getAllMatchedTuplesForEachStream(minTimestamp, minKey, maxKey, true);
            matchInputWithResultTuplesAndOutput(value, resultList, out);
