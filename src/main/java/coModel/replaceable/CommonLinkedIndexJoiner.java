...
import tools.index.DeleteConditionForSubIndex;

import java.io.Serializable;
import java.util.*;

/**
 * 通用的采用链式索引的Joiner的父类，其中Joiner的实现是可变的
 * @param <F> 第一个流的类型
 * @param <S> 第二个流的类型
 */
public abstract class CommonLinkedIndexJoiner<F, S> extends CommonJoiner<F, S> {
    ...
    protected List<CommonJoinerSubIndex<Double, CommonJoinUnionType<F, S>>> R_StoreBPlusTreeSet;
    protected List<CommonJoinerSubIndex<Double, CommonJoinUnionType<F, S>>> S_StoreBPlusTreeSet;
    ...
    protected final long subTreeTimeWindowInterval = CoModelParameters.SUB_INDEX_TIME_INTERVAL;
    ...
    protected final int minRemainMemory = CoModelParameters.SYSTEM_MAX_REMAIN_MEMORY;
    ...
    //获取logger对象用于日志记录
    protected static final Logger logger = Logger.getLogger(CommonLinkedIndexJoiner.class.getName());


    /**
     * 有参构造器，进行范围连接的相关初始化参数设置
     */
    public CommonLinkedIndexJoiner(Time r_TimeWindows, Time s_TimeWindows, KeySelector<F, Double> keySelector_R, KeySelector<S, Double> keySelector_S, double r_surpass_S, double r_behind_S) {
    ...
    protected void initStoreDataStructureForBothStream() {
        ...
        //初始化加入一个节点
        R_StoreBPlusTreeSet.add(creatOneNewSubIndex());
        S_StoreBPlusTreeSet.add(creatOneNewSubIndex());
    }

    /**
     * 需要子类重写的方法，在该方法中返回一个新的子索引的对象，
     * 子类重写该类时根据创建的对象类型的不同而实现不同类型的子索引
     * @return 新建的子索引对象
     */
    protected abstract CommonJoinerSubIndex<Double, CommonJoinUnionType<F, S>> creatOneNewSubIndex();
     ...
     * @param storeSubIndexSet 要被删除元组的子树集合
     */
    private void deleteAllOutOfRangeTuplesInSpecialSubTreeSet(
            Tuple2<Integer, Integer> localStoreSerialNumRange,
            List<CommonJoinerSubIndex<Double, CommonJoinUnionType<F, S>>> storeSubIndexSet) {

        long startTime = System.nanoTime();  //计时

        //删除每个子树中不满足序列号范围的元组
        for (CommonJoinerSubIndex<Double, CommonJoinUnionType<F, S>> subIndex : storeSubIndexSet) {
            subIndex.deleteNodeWithCondition(new DeleteConditionForSubIndex<CommonJoinUnionType<F, S>>() {
                @Override
        ...
        long finishTime = System.nanoTime();  //计时
        logger.info("CoModel-Joiner-" + subTaskIdx + ":删除本地的所有不在序列号范围内的元组成功！耗时（单位ns）：" + (finishTime - startTime) / 1000000);
    }
    ...
    private void insertTupleToEachStoreStructure(CommonJoinUnionType<F, S> value,
                                                 Double key,
                                                 long timestamp,
                                                 List<CommonJoinerSubIndex<Double, CommonJoinUnionType<F, S>>> storeBPlusTreeSet) {
        //将当前元组插入当前活动的子树
        CommonJoinerSubIndex<Double, CommonJoinUnionType<F, S>> currentActiveSubTree = storeBPlusTreeSet.get(0);
        currentActiveSubTree.insert(key, value, timestamp);
        ...
        if (currentActiveSubTree.getMaxTimestamp() - currentActiveSubTree.getMinTimestamp() > subTreeTimeWindowInterval) {
            ...
            storeBPlusTreeSet.add(0, creatOneNewSubIndex());
        }
    }
     ...
     * @param storeSubIndexSet 要查找的存储结构
     * @return 返回所有满足时间条件和键值范围的元组
     */
    protected List<CommonJoinUnionType<F, S>> getAllMatchedTuplesForEachStream(long minTimestamp,
                                                                               Double minKey,
                                                                               Double maxKey,
                                                                               List<CommonJoinerSubIndex<Double, CommonJoinUnionType<F, S>>> storeSubIndexSet) {
        ArrayList<CommonJoinUnionType<F, S>> resultList = new ArrayList<>();
        //遍历
        for (CommonJoinerSubIndex<Double, CommonJoinUnionType<F, S>> subTree : storeSubIndexSet) {
            if (subTree.getMaxTimestamp() < minTimestamp) {
                continue;
            }
            List<CommonJoinUnionType<F, S>> subResultList = subTree.findRangeCeilingMinTimestamp(minKey, true, maxKey, true, minTimestamp);
     ...
     */
    protected Tuple2<Long,Long> getStoreStatusOfEachStoreStructure(List<CommonJoinerSubIndex<Double, CommonJoinUnionType<F, S>>> storeBPlusTreeSet) {
        long subTreeNum = storeBPlusTreeSet.size();   //子树数量
        long totalTuplesNum = 0;    //总元组数量
        for (CommonJoinerSubIndex<Double, CommonJoinUnionType<F, S>> subTree : storeBPlusTreeSet) {
...
//    }
     ...
     * @param storeSubIndexSet 要执行过期操作的子树集
     * @param coMinTimestamp 对应子树应当保存的最小时间戳，应该由当前水位线和时间窗口共同计算出，即：水位线 - 时间窗口长度
     */
    private void expireEachStoreStructure(List<CommonJoinerSubIndex<Double, CommonJoinUnionType<F, S>>> storeSubIndexSet, long coMinTimestamp) {

        Iterator<CommonJoinerSubIndex<Double, CommonJoinUnionType<F, S>>> iterator = storeSubIndexSet.iterator();

        while (iterator.hasNext()) {
            //系统中应该至少有一个子索引，否则数组会越界
            if (storeSubIndexSet.size() == 1) {
                break;
            }

            //删除过期子索引
            CommonJoinerSubIndex<Double, CommonJoinUnionType<F, S>> next = iterator.next();
            if (next.getMaxTimestamp() < coMinTimestamp) {
                iterator.remove();
            }
        }//end while
