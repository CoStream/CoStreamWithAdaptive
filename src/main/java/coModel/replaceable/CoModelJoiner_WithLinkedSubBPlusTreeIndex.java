...
import tools.index.JoinerSubIndexWithBPlusTree;

/**
 * CoModel的Joiner，其中采用链式索引方法，且子索引是基于B+树实现的，
 * 并且在该类中会记录存储和连接元组的执行时间并上传
 * @param <F> 第一个流的类型
 * @param <S> 第二个流的类型
 */
public class CoModelJoiner_WithLinkedSubBPlusTreeIndex<F, S> extends CommonLinkedIndexJoiner<F, S> {
     ...
     */
    public CoModelJoiner_WithLinkedSubBPlusTreeIndex(Time r_TimeWindows, Time s_TimeWindows, KeySelector<F, Double> keySelector_R, KeySelector<S, Double> keySelector_S, double r_surpass_S, double r_behind_S) {
    ...
    protected CommonJoinerSubIndex<Double, CommonJoinUnionType<F, S>> creatOneNewSubIndex() {
        logger.info("CoModel-Joiner-创建了一个新的子索引，子索引的类型是SubBPlusTree。");
        return new JoinerSubIndexWithBPlusTree<>();
    }
    ...
    public void open(Configuration parameters) throws Exception {
        ...
        logger.info("当前启动的是内部子索引结构为 Sub B+ Tree 的CoModel的Joiner！");
