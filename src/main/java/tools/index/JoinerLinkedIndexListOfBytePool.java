...
/**
 * 由所有基于字节数组内存池的内存池子索引构成的子索引列表
 * @param <F> 第一个流的类型
 * @param <S> 第二个流的类型
 * @param <K> 存储的键值类型
 */
public class JoinerLinkedIndexListOfBytePool<F, S, K extends Comparable<K>> implements Serializable {
    //最大子索引数量
    private int maxSubIndexNum;
    //每个子索引的最大容量(字节数)
    private int maxByteCapacityOfSubIndex;
    //每个子索引存储元组的时间范围
    private long timeIntervalOfSubIndex;
    //子索引链表
    private List<JoinerSubIndexWithBytePool<F, S, K>> subIndexLinkedList;
    //当前活动子索引位置
     ...
     */
    public JoinerLinkedIndexListOfBytePool(int maxSubIndexNum, int maxByteCapacityOfSubIndex, long timeIntervalOfSubIndex) {
        ...
        for (int i = 0; i < maxSubIndexNum; i++) {
            subIndexLinkedList.add(new JoinerSubIndexWithBytePool<>(maxByteCapacityOfSubIndex));
        }
        //设置当前活动子索引
        activeSubIndexPosition = 0;
        System.out.println("当前使用的是基于字节数组的内存池-JoinerLinkedIndexListOfBytePool！");
    }
    ...
    public int getMaxByteCapacityOfSubIndex() {
    ...
    public List<CommonJoinUnionType<F, S>> findRangeCeilingMinTimestamp(K fromKey, boolean fromInclusive, K toKey, boolean toInclusive, long minTimestamp){
        ...
        for (JoinerSubIndexWithBytePool<F, S, K> subIndex : subIndexLinkedList) {
     ...
     */
    public List<Tuple4<Integer, Integer, Integer, Long>> getStoreStatusOfAllSubIndex() {
        ArrayList<Tuple4<Integer, Integer, Integer, Long>> resultList = new ArrayList<>(maxSubIndexNum);
        for (JoinerSubIndexWithBytePool<F, S, K> subIndex : subIndexLinkedList) {
            resultList.add(new Tuple4<>(subIndex.getSize(), subIndex.getByteSize(), subIndex.getMaxCapacity(), subIndex.getMaxTimestamp()));
        }
        return resultList;
    }
